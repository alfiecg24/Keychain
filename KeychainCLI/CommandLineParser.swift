...
//  Fugu
//
//  Created by Linus Henze on 14.10.19 for Fugu
//  Edited and used for Keychain by alfiecg24
//
import Foundation
...
fileprivate func getModuleDescription(module: CommandLineModule.Type, appName: String) -> String {
    ...
    for i in module.requiredArguments {
        ...
        if i.shortVersion != "" {
            ...
            print("getModuleDescription: XXX - This shouldn't happen")
            exit(1)
        }
    }
    ...
    if module.optionalArguments.count != 0 {
        ...
        for i in module.optionalArguments {
            ...
            if i.name != "" && i.shortVersion == "" && i.longVersion == "" {
                print("getModuleDescription: A short or long version must be set for optional parameters, not a name.")
                exit(1)
            } else {
                if type != "" {
                    result += " <\(type)>"
